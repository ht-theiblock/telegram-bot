...
if not GROQ_API_KEY:
    raise ValueError("❌ Chưa tìm thấy GROQ_API_KEY!")
...
def call_groq(user_id: int, user_message: str) -> tuple[str, bool]:
    history = get_history(user_id)
    searched = False
...
async def handle_message(update: Update, context):
    ...
    try:
        reply, searched = await asyncio.to_thread(call_groq, user_id, user_message)
        if not reply:
